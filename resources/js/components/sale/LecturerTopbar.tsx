...
export default function LecturerTopbar({ onOpenMobileMenu }: LecturerTopbarProps) {
    const { url } = usePage();
    ...
    const getPageTitle = () => {
        if (url.includes('/dosen/courses')) return 'My Courses';
        if (url.includes('/dosen/curriculum')) return 'RPS / CPMK / CPL';
        if (url.includes('/dosen/materials')) return 'Materials & Module';
        if (url.includes('/dosen/assignments')) return 'Assignments & Quizzes';
        if (url.includes('/dosen/gradebook')) return 'Gradebook';
        if (url.includes('/dosen/attendance')) return 'Attendance';
        if (url.includes('/dosen/analytics')) return 'Class Analytics';
        if (url.includes('/dosen/forum')) return 'Discussion Forum';
        if (url.includes('/dosen/notifications')) return 'Notifikasi';
        if (url.includes('/dosen/profile')) return 'Profil Dosen';
